...
{
	/// <summary>
	/// Description of C184762_UsingCPUwiget_TotalinDBHighlights.
	/// </summary>
	[TestModule("E7E8BE1B-5595-4A54-A68D-A5EE201786C8", ModuleType.UserCode, 1)]
	public class C184762_UsingCPUwiget_TotalinDBHighlights : ITestModule
	{
		/// <summary>
		/// Constructs a new instance.
		/// </summary>
		public C184762_UsingCPUwiget_TotalinDBHighlights()
		{
		...
		void ITestModule.Run()
		{
			...
			try{
				...
				IList<InputTag> timeSelection = commonRepo.IderaDashboard.BC2Un2.timeDelaySelection.FindDescendants<InputTag>();//4W
				...
				Report.Log(ReportLevel.Info,"Click on Using CPU in DB Highlights");
				ATag usingCpuPath  = "/dom[@domain='localhost:9291']//iframe[1]//table[#'entityOverviewDataTable']//a[@innertext='Using CPU']";
				
				usingCpuPath.PerformClick();
				Report.Screenshot();
				Delay.Seconds(3);
				//commonRepo.IderaDashboard.BC2Un2.Content.MoveTo();
				commonRepo.IderaDashboard.BC2Un2.Content.Click();
				
				var cpuUtilizationOverTime = commonRepo.IderaDashboard.BC2Un2.CpuUtilizationOverTime;
				Validate.AttributeEqual(commonRepo.IderaDashboard.BC2Un2.CpuUtilizationOverTimeInfo,"Visible","True");
				Report.Log(ReportLevel.Info,"Cpu utilization graph is loaded");
				

				SWAWebCommon.Scroll(commonRepo.IderaDashboard.BC2Un2.TopStatementsInfo,"down");
				Delay.Seconds(2);
